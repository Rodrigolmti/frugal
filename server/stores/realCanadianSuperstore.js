...
class RealCanadianSuperstore extends BaseStore {
  ...
  async searchProducts(searchTerm) {
    let browser;
    try {
      console.log(`🔍 Starting Puppeteer search for: "${searchTerm}"`);
      ...
      // First, let's try to access the homepage to test connectivity
      console.log(`🏠 Testing homepage access first...`);
      try {
        ...
        console.log(`❌ Cannot access homepage: ${homeError.message}`);
        throw new Error(`Website appears to be inaccessible: ${homeError.message}`);
      }

      // Navigate to search URL
      const searchUrl = `${this.baseUrl}/search?search-bar=${encodeURIComponent(searchTerm)}`;
      console.log(`🌐 Now navigating to search: ${searchUrl}`);
      ...
      try {
        ...
        try {
          await page.waitForSelector('input[type="search"], input[name*="search"], #search', { timeout: 10000 });
          await page.type('input[type="search"], input[name*="search"], #search', searchTerm);
      ...
      const products = await page.evaluate(() => {
        ...
        // Strategy 1: Look for Real Canadian Superstore product grid structure
        // Use only the most specific selector to avoid duplicates
        ...
        possibleProductElements.forEach((element, index) => {
          try {
            ...
            // Real Canadian Superstore specific selectors based on actual HTML structure
            const nameSelectors = [
              '[data-testid="product-title"]', // Main product title
              'h3[data-testid="product-title"]', // Specific h3 with product title
              '.chakra-heading[data-testid="product-title"]' // Chakra heading with product title
            ];
            
            const priceSelectors = [
              '[data-testid="regular-price"]', // Regular price
              '[data-testid="sale-price"]', // Sale price
              '[data-testid="price-product-tile"] span' // Any price in price tile
            ];
            
            const brandSelectors = [
              '[data-testid="product-brand"]' // Product brand
            ];
            
            const packageSizeSelectors = [
              '[data-testid="product-package-size"]' // Package size info
            ];
            ...
            const imgEl = element.querySelector('[data-testid="product-image"] img, .chakra-image');
            if (imgEl) {
              image = imgEl.src || imgEl.getAttribute('data-src') || imgEl.getAttribute('srcset')?.split(' ')[0] || '';
            }

            // Find product link - look for the overlay link
            const linkEl = element.querySelector('.chakra-linkbox__overlay, a[href*="/p/"]');
            if (linkEl) {
            ...
            const wasPriceEl = element.querySelector('[data-testid="was-price"]');
      ...
      const seenProducts = new Set(); // Track products we've already processed
            ...
            if (price > 0) {
              ...
              const product = {
                ...
                image: rawProduct.image && rawProduct.image.startsWith('http') ? rawProduct.image : 
                       rawProduct.image ? `${this.baseUrl}${rawProduct.image}` : '',
                url: rawProduct.link && rawProduct.link.startsWith('http') ? rawProduct.link :
                     rawProduct.link ? `${this.baseUrl}${rawProduct.link}` : '',
                inStock: true,
      ...
      console.error('❌ Error searching Real Canadian Superstore:', error.message);
